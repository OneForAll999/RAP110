CLASS lsc_zrap110_r_traveltp_02a DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zrap110_r_traveltp_02a IMPLEMENTATION.
  ...
  METHOD adjust_numbers.
    ...
    IF mapped-travel IS NOT INITIAL.
      ...
      TRY.
          ...
          EXPORTING nr_range_nr = '01' object = 'ZRAP11002A' quantity = CONV #( lines( mapped-travel ) )
          IMPORTING number = DATA(number_range_key) returncode = DATA(number_range_return_code) returned_quantity = DATA(number_range_returned_quantity) ).

        CATCH cx_number_ranges INTO DATA(lx_number_ranges) .
          RAISE SHORTDUMP TYPE cx_number_ranges EXPORTING previous = lx_number_ranges.
      ENDTRY.
    ...
    IF mapped-booking IS NOT INITIAL.
      ...
      LOOP AT mapped-booking INTO DATA(mapped_booking) GROUP BY mapped_booking-TravelID.
        SELECT MAX( booking_id ) FROM zrap110_abook02a WHERE travel_id = @mapped_booking-TravelID INTO @DATA(max_booking_id) .
  ...
  METHOD save_modified.
    ...
    IF update IS NOT INITIAL.
      ...
      RAISE ENTITY EVENT zrap110_r_traveltp_02a~travel_accepted
      FROM VALUE #( FOR travel IN update-travel WHERE ( %control-OverallStatus EQ if_abap_behv=>mk-on AND OverallStatus EQ travel_status-accepted )
          ( %key           = travel-%key
      ...
      "raise event
      RAISE ENTITY EVENT ZRAP110_R_TravelTP_02a~travel_rejected
       FROM VALUE #(
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF ZRAP110_R_TravelTP_02a IN LOCAL MODE
    ENTITY travel
    UPDATE FIELDS ( OverallStatus )
    WITH VALUE #( FOR key IN keys ( %tky = key-%tky OverallStatus = travel_status-accepted ) ).
  ...
  ENTITY travel
  UPDATE FIELDS ( OverallStatus )
  WITH VALUE #( FOR key IN keys ( %tky = key-%tky OverallStatus = travel_status-rejected ) ).
  ...
  METHOD setInitialTravelValues.
    ...
    DATA: update TYPE TABLE FOR UPDATE zrap110_r_traveltp_02a\\Travel.
